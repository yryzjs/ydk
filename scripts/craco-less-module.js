...
const overrideWebpackConfig = ({context, webpackConfig, pluginOptions}) => {
  ...
  const throwError = (message, githubIssueQuery) =>
    throwUnexpectedConfigError({
      packageName: 'craco-less-module',
      githubRepo: 'ydk-web/craco-less-module',
      message,
      githubIssueQuery,
    })
  ...
  if (!sassRule) {
    throwError(
      "Can't find the webpack rule to match module scss/sass files in the " +
        `${context.env} webpack config!`,
      'webpack+rules+module+scss+sass',
    )
  }
  let lessRule = {
    test: lessExtension,
  ...
  loaders.forEach((ruleOrLoader) => {
    ...
    if (
      ...
      console.warn('rule.options', rule.options)
